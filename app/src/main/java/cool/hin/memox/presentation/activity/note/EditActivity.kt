...
import cool.hin.memox.presentation.activity.main.MainActivity.Companion.EXTRA_FRAGMENT_TO_OPEN
import cool.hin.memox.presentation.activity.main.MainActivity.Companion.EXTRA_SKIP_START_VIEW_ON_BACK
...
import cool.hin.memox.presentation.displayEditLabelDialog
...
import cool.hin.memox.presentation.view.note.ErrorAdapter
import cool.hin.memox.presentation.view.note.action.ActionSelectionBottomSheet
import cool.hin.memox.presentation.view.note.action.AddBottomSheet
import cool.hin.memox.presentation.view.note.action.MoreNoteBottomSheet
import cool.hin.memox.presentation.view.note.audio.AudioAdapter
import cool.hin.memox.presentation.view.note.preview.PreviewFileAdapter
import cool.hin.memox.presentation.view.note.preview.PreviewImageAdapter
...
import cool.hin.memox.presentation.viewmodel.preference.editBodySize
import cool.hin.memox.presentation.viewmodel.preference.editTitleSize
import cool.hin.memox.presentation.viewmodel.preference.isAutoSortChecked
import cool.hin.memox.presentation.widget.WidgetProvider
import cool.hin.memox.utils.FileError
import cool.hin.memox.utils.changeStatusAndNavigationBarColor
import cool.hin.memox.utils.changehistory.ChangeHistory
import cool.hin.memox.utils.findWebUrls
import cool.hin.memox.utils.getUriForFile
import cool.hin.memox.utils.isInLandscapeMode
import cool.hin.memox.utils.log
import cool.hin.memox.utils.mergeSkipFirst
import cool.hin.memox.utils.observeSkipFirst
import cool.hin.memox.utils.security.showBiometricOrPinPrompt
import cool.hin.memox.utils.textMaxLengthFilter
...
abstract class EditActivity(private val type: Type) : LockedActivity<ActivityEditBinding>() {
    ...
    internal val notallyModel: MemoXModel by viewModels()
    ...
    private fun showActionSelectionDialog(
        ...
        ActionSelectionBottomSheet(
                ...
                onReset = {
                    val prefs = MemoXPreferences.getInstance(this)
                    if (isBottomBar) {
                        prefs.editNoteActivityBottomAction.save(
                            MemoXPreferences.DEFAULT_EDIT_NOTE_BOTTOM_ACTION
                        )
                    } else {
                        val currentActions =
                        ...
                        if (index in currentActions.indices) {
                            currentActions[index] =
                                MemoXPreferences.DEFAULT_EDIT_NOTE_TOP_ACTIONS[index]
    ...
    protected open fun openMoreOptionsBottomSheet() {
        val prefs = MemoXPreferences.getInstance(this@EditActivity)
     ...
     * [IMAGE_PLACEHOLDER] with an [cool.hin.memox.presentation.InlineImageSpan]; the
     * resulting text change goes through the change history (undoable) and triggers
     * [MemoXModel.syncImagesFromBody] to keep the `images` list in sync.
     */
     ...
     * (undoable) and triggers [MemoXModel.syncImagesFromBody] so the `images` list and database
    ...
    private fun displayFileErrors(errors: List<FileError>) {
        ...
        val message =
            if (errors.isNotEmpty() && errors[0].fileType == MemoXModel.FileType.IMAGE) {
    ...
    companion object {
        ...
        const val EXTRA_SELECTED_BASE_NOTE = "memox.intent.extra.SELECTED_BASE_NOTE"
        const val EXTRA_SELECTED_LABELS = "memox.intent.extra.SELECTED_LABELS"
        const val EXTRA_NOTE_ID = "memox.intent.extra.NOTE_ID"
