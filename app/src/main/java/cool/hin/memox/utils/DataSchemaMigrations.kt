...
import cool.hin.memox.utils.NoteRepairUtils.truncateBodyAndFixSpans
import cool.hin.memox.utils.NoteSplitUtils.splitOversizedExistingNoteForMigration
...
suspend fun Application.runMigrations(onProgressTitle: (Int) -> Unit = {}): Boolean {
    ...
    withContext(Dispatchers.IO) {
        if (dataSchemaId < 1) {
            onProgressTitle(cool.hin.memox.R.string.migration_moving_attachments)
            moveAttachments(preferences)
            newDataSchemaId = 1
            didWork = true
        }
        if (newDataSchemaId < 2) {
            onProgressTitle(cool.hin.memox.R.string.migration_splitting_notes)
            splitOversizedNotes()
            newDataSchemaId = 2
            didWork = true
        }
...
private fun Application.moveAttachments(preferences: MemoXPreferences) {
...
suspend fun Application.splitOversizedNotes() {
    ...
    val db = MemoXDatabase.getDatabase(this as ContextWrapper, false).value
